...
function recherche(){
	...
	for(let wine of wines) {
		if(wine.name.toLowerCase() == document.getElementById("motClef").value.toLowerCase()){
			...
			//document.getElementById("picture").setAttribute("src", "assets/pics/"+wine.picture);
	  ...
	  //DETAIL D'UN SEUL VIN
	    function clickWine (idVinChoisi) {
			...
			for(let i = 0; i < wines.length; i++){
				...
				//document.getElementById("picture").setAttribute("src", "assets/pics/"+wines[i].picture);
...
// fetch('https://cruth.phpnet.org/epfc/caviste/public/index.php/api/wines')
	  ...
	  // .then(function(json) { 
			// wines = json;
          // //document.write(wines[2].name)
		  // document.getElementById("vin").innerHTML = wines[2].name;
	  // })
